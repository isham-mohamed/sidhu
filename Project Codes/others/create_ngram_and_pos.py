...
def ngram_pos(filename):    
    ...
    for each_sentence in clean_data[:]:
        ...
        filepath = "D://Sem 8/Seminar and Project/Project Work/hate_pos/file_" + str(i) + ".txt"
        f = open(filepath, "w")
        for j in range(len(pos_tokens)-1):
            f.write(pos_tokens[j][1]+" "+pos_tokens[j+1][1]+"\n")
        f.close()
        #i+=1 
        
        
        
        filepath = "D://Sem 8/Seminar and Project/Project Work/hate_ngram/file_" + str(i) + ".txt"
        f = open(filepath, "w")
        ngram_tokens = list(ngrams(tokens,2))
        for ele in ngram_tokens[:]:
            for e in ele[:]:
                f.write(e+" ")
            f.write(" \n")
            #simplejson.dump(t, f)
...
ngram_pos("hate_clean.csv")
#ngram_pos("offensive_clean.csv")
#ngram_pos("neutral_clean.csv")
